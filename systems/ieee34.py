...
class IEEE34BusSystem(gym.Env):
    ...
    def __init__(self, **kwargs):
        ...
        self.datasize = self.dataset['solar'].size
    ...
    def reset(self, day=None, seed=None):
        ...
        t, w, data = self.t, self.window, self.data
        if t-w >= 0:
            # self.past_load_0 = deque(data['load_0'][t-w:t], maxlen=w)
            # self.past_load_1 = deque(data['load_1'][t-w:t], maxlen=w)
            # self.past_load_2 = deque(data['load_2'][t-w:t], maxlen=w)
            # self.past_wind_0 = deque(data['wind_0'][t-w:t], maxlen=w)
            # self.past_wind_1 = deque(data['wind_1'][t-w:t], maxlen=w)
            # self.past_wind_2 = deque(data['wind_2'][t-w:t], maxlen=w)
            # self.past_solar_0 = deque(data['solar_0'][t-w:t], maxlen=w)
            # self.past_solar_1 = deque(data['solar_1'][t-w:t], maxlen=w)
            # self.past_solar_2 = deque(data['solar_2'][t-w:t], maxlen=w)
            self.past_load = deque(data['load'][t-w:t], maxlen=w)
            self.past_wind = deque(data['wind'][t-w:t], maxlen=w)
            self.past_solar = deque(data['solar'][t-w:t], maxlen=w)
            self.past_price = deque(data['price_sigmoid'][t-w:t], maxlen=w)
            ...
            # self.past_load_0 = deque(np.hstack([data['load_0'][t-w:], data['load_0'][:t]]), maxlen=w)
            # self.past_load_1 = deque(np.hstack([data['load_1'][t-w:], data['load_1'][:t]]), maxlen=w)
            # self.past_load_2 = deque(np.hstack([data['load_2'][t-w:], data['load_2'][:t]]), maxlen=w)
            # self.past_wind_0 = deque(np.hstack([data['wind_0'][t-w:], data['wind_0'][:t]]), maxlen=w)
            # self.past_wind_1 = deque(np.hstack([data['wind_1'][t-w:], data['wind_1'][:t]]), maxlen=w)
            # self.past_wind_2 = deque(np.hstack([data['wind_2'][t-w:], data['wind_2'][:t]]), maxlen=w)
            # self.past_solar_0 = deque(np.hstack([data['solar_0'][t-w:], data['solar_0'][:t]]), maxlen=w)
            # self.past_solar_1 = deque(np.hstack([data['solar_1'][t-w:], data['solar_1'][:t]]), maxlen=w)
            # self.past_solar_2 = deque(np.hstack([data['solar_2'][t-w:], data['solar_2'][:t]]), maxlen=w)
            self.past_load = deque(np.hstack([data['load'][t-w:], data['load'][:t]]), maxlen=w)
            self.past_wind = deque(np.hstack([data['wind'][t-w:], data['wind'][:t]]), maxlen=w)
            self.past_solar = deque(np.hstack([data['solar'][t-w:], data['solar'][:t]]), maxlen=w)
            self.past_price = deque(np.hstack([data['price_sigmoid'][t-w:], data['price_sigmoid'][:t]]), maxlen=w)
    ...
    def _get_obs(self):
        ...
        # window, net = self.window, deepcopy(self.net)
        # # past demand at every bus
        # df = pd.DataFrame(columns=['bus','area']+['{}'.format(t) for t in range(window)])
        # df.bus = net.load.bus
        ...
        # past_p_mw = dfn.groupby(dfn.index).agg(dict(zip(['{}'.format(t) for t in range(window)], [sum]*window))).values
